...
tar -xvf latest.tar.gz && mv wordpress var/www/html/
...
mv phpMyAdmin-5.0.2-all-languages phpmyadmin && mv phpmyadmin /var/www/html/
cp -rp /tmp/config.inc.php var/www/html/phpmyadmin/

rm phpMyAdmin-5.0.2-all-languages.tar.gz && rm latest.tar.gz

if [ "$AUTOINDEX" = "off" ] ;
then cp /tmp/index_off /etc/nginx/sites-available/default ;
else cp /tmp/index_on /etc/nginx/sites-available/default ;
fi
